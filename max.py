...
def transfer(filename):
    ...
    to_id = []  # 用于存储转换后的to_id
    # print(result)
    data_ = pd.read_csv("data/address_to_id_max_label.csv")  # 读取存储投资者的address对应id的文件
...
def feature(filename):
    ...
    for i in range(0, len(token_address)):
        if token_address[i] == address:
            ...
            #    value_to.append(x)
            with open('data/transfer_max_label.csv', 'a+', encoding='utf-8', newline='') as f:
                writer = csv.writer(f)
...
def load_edge(filename):
    ...
    for i in range(0, len(token_id)):
        ...
        timestamp.append(block_time[i])
    with open('data/data_max_label.csv','w',encoding='utf-8',newline='') as f:
        writer = csv.writer(f)
...
if __name__=='__main__':
    ...
    print(new_li)
    with open('data/address_to_id_max_label.csv', 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['address', 'id'])
        for i in range(0, len(new_li)):
            writer.writerow([new_li[i], i])
    transfer("data/transfer_max_label")
    load_edge("data/transfer_max_label_new.csv")
