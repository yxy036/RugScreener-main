...
class MsgLinkPredictor(nn.Module):
    ...
    Parameters
    使用消息传递从pos-sub和neg-subg预测成对链接。使用边缘上的两层MLP预测链路概率参数
    ----------
    embed_dim : int
...
class TimeEncode(nn.Module):
    ...
    使用不同相位和频率的有限傅里叶级数进行编码两个事件之间的时间差
    ..math::
...
class MemoryModule(nn.Module):
    ...
    def reset_memory(self):
        self.last_update_t = nn.Parameter(torch.zeros(
            self.n_node).float().to(device), requires_grad=False)
        self.memory = nn.Parameter(torch.zeros(
            (self.n_node, self.hidden_dim)).float().to(device), requires_grad=False)
...
class MemoryOperation(nn.Module):
    ...
    def stick_feat_to_graph(self, g):
        ...
        g.ndata['timestamp'] = self.memory.last_update_t[g.ndata[dgl.NID]].to(device)
        g.ndata['memory'] = self.memory.memory[g.ndata[dgl.NID]].to(device)
...
class EdgeGATConv(nn.Module):
    ...
    edge feature.
    边缘图注意力从节点和边缘特征计算图注意力，然后聚合节点和边缘特性。
    Parameter
    ==========
    node_feats : int
...
class TemporalTransformerConv(nn.Module):
    def __init__(self,
        ...
        Parameter
        ==========
        edge_feats : int，边缘特征数
        memory_feats : int，记忆向量维数
        temporal_encoder : torch.nn.Module，计算傅里叶时间编码
        out_feats : int，输出特征数
        num_heads : int，注意头数量
        allow_zero_in_degree : bool, optional，如果图中有0度节点，则这些节点的输出将无效，因为不会向这些节点传递任何消息。
        这对于某些导致静默性能回归的应用程序是有害的。如果该模块在输入图中检测到0度节点，则会引发DGLError。通过设置“True”，它将抑制检查并让用户自行处理。默认值：``False``。
